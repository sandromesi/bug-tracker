...
const Comments = ({ issue }) => {
    ...
    useEffect(() => {
        fetch(`${process.env.REACT_APP_BASE_URL}issues/${issue.id}/comments`)
            .then(res => res.json())
            .then(res => {
                setComments(res)
            })
                        ...
                        <tr key={index}>
                            ...
                            <td className="d-flex justify-content-center">
                                <div className='p-2 align-middle'>
                                    <svg type="button"
                                        onClick={() => { setPutCommentModal(true); setComment(comment) }}
                                    ...
                                    {PutCommentModal && <PutComment comment={oneComment} closeCommentModal={setPutCommentModal} />}
                                </div>
                                <div className='p-2 align-middle'>
                                    <svg
                                        type="button"
                                        onClick={() => { setDeleteCommentModal(true); setComment(comment) }}
                                    ...
                                    {DeleteCommentModal && <DeleteComment comment={oneComment} closeCommentModal={setDeleteCommentModal} />}
                                </div>
                            </td>
                        </tr>
                    ))}
